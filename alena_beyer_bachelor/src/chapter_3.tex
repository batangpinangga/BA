...
\section{Large Data}:
In visualizing large data there appears a number of challenges. 
...
\\*
\textit{Overlap}: 
If the whole dataset is visualized or even a subset the data may overlap and reduce visual clutter. Visual clutter hinders the user in the completion of its tasks.
\\*
\textit{Visual Noise}: 
...
As data collection nowadays is done on the fly, with smartphones, sensors etc. the amount of available data is massive. Even if large monitors are used to visualize data in the end the available pixels on the monitor is smaller than the number of data items in a dataset. 
\\*
\textit{Limited visual perception}:
Moreover, human perception is also limited.
...
Hereby, we assume that interaction is in responsibility of the visualization tools. The most important interaction techniques are: 
